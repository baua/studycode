...
(* DONE add shifts for expressions of the form Shift(deltaX, deltaY, exp *)
...
fun eval_prog (e,env) =
	...
	(case List.find (fn (s2,v) => s=s2) env of
      ...
      | Shift(dx,dy,e1) => case eval_prog(e1,env) of
                              ...
                              | LineSegment(p1,p2,p3,p4) => LineSegment(p1+dx,p2+dy,p3+dx,p4+dy)
                              | _ => raise Impossible "This is no type value"

(* DONE: Add a case for Shift expressions *)

(* DONE: Add function preprocess_prog of type geom_exp -> geom_exp *)
fun preprocess_prog e =
    case e of
        LineSegment(p1,p2,p3,p4) => if real_close(p1,p3) andalso real_close(p2,p4)
                                    then
                                        Point(p1,p2)
                                    else
                                        if p1 < p3 then
